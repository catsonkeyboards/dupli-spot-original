...
import { resetUI, updateRemoveDuplicatesButtonState } from "./uiFunctions.js";
...
let currentOffset = importedOffset;
...
document.getElementById("login-button").addEventListener("click", function () {
  ...
  const scope =
    "playlist-modify-public playlist-modify-private playlist-read-private playlist-read-collaborative"; // Spotify API scopes to include in our Privacy Policy
  const authUrl = `https://accounts.spotify.com/authorize?client_id=9fdba1a5111447ebad9b2213859f814a&response_type=token&scope=${encodeURIComponent(
    scope
  )}&redirect_uri=https://dupli-spot-original.vercel.app/redirect.html`;
  ...
  const handleCallback = (event) => {
    ...
    showDuplicatesButton.addEventListener(
      "click",
      handleShowDuplicatesButtonClick.bind(showDuplicatesButton)
    );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.body.addEventListener(
    "input",
    Utility.debounce(function (event) {
  ...
  document
    ...
    .addEventListener("change", function (event) {
      const allDuplicateCheckboxes = document.querySelectorAll(
        '#duplicates input[name="duplicate"]'
      );
...
function pauseCurrentAudio() {
  ...
  document.addEventListener("click", function (event) {
    // Define loadingGraphic before using it
    const loadingGraphic = document.getElementById("loading-graphic");
      ...
      if (targetElement.id === "remove-duplicates") {
        ...
        removeDuplicatesFromPlaylist(selectedPlaylistId, selectedDuplicates)
          .then(() => {
            ...
            console.log("Clearing selectedDuplicates array.");
            ...
            console.log(
              "Fetching and comparing tracks for:",
              playlist1Id,
              playlist2Id
            );
          ...
          .then((duplicates) => {
            console.log(
              "Successfully fetched and compared tracks. Duplicates found:",
              duplicates.length
            );
            ...
            const playlist2Id = selectedPlaylists[1];
            console.log(
              "Fetching details for playlists:",
              playlist1Id,
              playlist2Id
            );
            return Promise.all([
              fetchPlaylistDetails(playlist1Id),
              fetchPlaylistDetails(playlist2Id),
            ]);
          })
          .then(([playlist1Details, playlist2Details]) => {
            console.log("Successfully fetched details for both playlists.");
            ...
            document.getElementById("start-over-button").style.display =
              "block";

            // Update the dropdown options with the new playlist names and track counts
            const dropdown = document.getElementById(
              "removal-playlist-dropdown"
            );
            dropdown.innerHTML = `
        ...
        `;
          ...
          .catch((error) => {
            ...
            errorMessageDiv.textContent =
              "Failed to remove duplicates. Ensure you have the necessary permissions for this playlist.";
